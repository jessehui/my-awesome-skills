...
| 版本/API/兼容性 | 本地文档（如有） | 联网搜索官方文档 |
| 因果推断 | 本地实验数据/文档 | 联网搜索（需 ≥1 个独立来源） |
| 技术建议 | 本地配置和历史 | 联网搜索官方文档 |

### 检索工具选择

联网检索时，按以下顺序选择工具：

1. **默认 search 工具**：优先使用环境默认的搜索工具（Claude Code 中为 `WebSearch`，必要时配合 `WebFetch` 取页面正文）。
2. **降级到其他 websearch 工具**：默认工具不可用时（未启用、报错、无权限），尝试环境中其他可用的联网搜索/浏览工具（如已配置的 search 类 MCP 工具）。
3. **无任何 websearch 工具时**：**必须给出特别告警**——明确告知用户「当前环境无可用联网搜索工具，以下断言仅基于本地来源与模型已有知识，未经联网核实」，并将相关断言标注为 `❗未验证` 或 `⚠️部分验证`，不得伪装成已联网验证。
